...
constexpr const int MAX_PENDING_SAMPLES = 3;
constexpr const int EVENT_WINDOW = 10;
...
public:
    ...
    using CBHandler::pending_samples_;
    using CBHandler::cb_writer_;
};

TEST(DdsHelperParticipantsTest, ddshelper_participants_cb_handler_creation)
{
    // Create Payload Pool
    ...
    CBHandlerConfiguration handler_config(MAX_PENDING_SAMPLES, EVENT_WINDOW);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_new_schemas)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_same_type_id_schema)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_same_type_name_schema)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_data_with_schema)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_data_without_schema_no_pending)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
    ...
    // Create CB Handler configuration
    CBHandlerConfiguration handler_config(0, EVENT_WINDOW);
    ...
    ASSERT_TRUE(cb_handler_->pending_samples_.empty());
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 0);
    ASSERT_NO_THROW(cb_handler_->add_data(topic, *data));
    ASSERT_TRUE(cb_handler_->pending_samples_.empty());
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 0);
}

TEST(DdsHelperParticipantsTest, ddshelper_participants_add_data_without_schema_with_max_pending)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
    ...
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 0);
    ASSERT_NO_THROW(cb_handler_->add_data(topic, *data));
    ASSERT_FALSE(cb_handler_->pending_samples_.empty());
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 1);
    ASSERT_EQ(cb_handler_->pending_samples_[topic.type_name].size(), 1);
}

TEST(DdsHelperParticipantsTest, ddshelper_participants_add_data_without_schema_over_max_pending)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
    ...
    ASSERT_NO_THROW(cb_handler_->add_data(topic, *data));
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 1);
    ASSERT_EQ(cb_handler_->pending_samples_[topic.type_name].size(), 2);

    ASSERT_NO_THROW(cb_handler_->add_data(topic, *data));
    ASSERT_EQ(cb_handler_->pending_samples_.size(), 1);
    ASSERT_EQ(cb_handler_->pending_samples_[topic.type_name].size(), 3);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_add_schema_with_pending_samples)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_write_schema_first_time)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
...
TEST(DdsHelperParticipantsTest, ddshelper_participants_write_schema_repeated)
{
    // Create Payload Pool
    auto payload_pool_ = std::make_shared<ddspipe::core::FastPayloadPool>();
    ASSERT_NE(payload_pool_, nullptr);
